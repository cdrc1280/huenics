            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div class="lg:col-span-8 space-y-6">
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                        <div class="p-5 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-slate-50/50">
                            <div>
                                <h2 class="text-base font-bold text-slate-900 flex items-center gap-2">
                                    <i class="fa-solid fa-list-check text-blue-600"></i>
                                    <span>Selected Line Items</span>
                                </h2>
                                <p class="text-xs text-slate-500 mt-0.5">Edit quantities or unit prices directly in the table.</p>
                            </div>

                            <div class="flex items-center gap-2 w-full sm:w-auto">
                                <button type="button" 
                                        onclick="openAddCatalogModal()" 
                                        class="flex-1 sm:flex-initial bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 text-xs font-bold px-3 py-2 rounded-lg transition flex items-center justify-center gap-1.5">
                                    <i class="fa-solid fa-cart-plus"></i>
                                    <span>Add from Catalog</span>
                                </button>
                                <button type="button" 
                                        onclick="addCustomRow()" 
                                        class="flex-1 sm:flex-initial bg-slate-100 text-slate-800 hover:bg-slate-200 border border-slate-300 text-xs font-bold px-3 py-2 rounded-lg transition flex items-center justify-center gap-1.5">
                                    <i class="fa-solid fa-plus"></i>
                        ...
                        <div id="empty-cart-state" class="hidden text-center py-12 px-4">
                            ...
                            <h3 class="text-sm font-bold text-slate-800">Your quotation estimate is currently empty</h3>
                            <p class="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
                                Add products from our catalog or enter custom line items to compute your project totals.
                            </p>
                            <div class="mt-4 flex justify-center gap-3">
                                <button type="button" onclick="openAddCatalogModal()" class="bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs px-4 py-2 rounded-lg transition">
                                    <i class="fa-solid fa-boxes-stacked mr-1"></i> Browse Catalog
                                </button>
                                <button type="button" onclick="addCustomRow()" class="bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs px-4 py-2 rounded-lg transition border border-slate-300">
                        ...
                        <div class="p-3 bg-slate-50 border-t border-slate-200 flex justify-between items-center text-xs">
                            <span class="text-slate-500" id="table-row-count">0 items selected</span>
                            <button type="button" onclick="clearAllItems()" class="text-red-600 hover:text-red-800 font-semibold hover:underline">
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
                        <div class="border-b border-slate-200 pb-3">
                            ...
                            <p class="text-xs text-slate-500 mt-0.5">These details will appear on the generated quotation header.</p>
                        </div>
                ...
                <div class="lg:col-span-4 space-y-6 sticky top-24">
                    ...
                    <!-- Calculation Card -->
                    <div class="bg-slate-900 text-white border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
                        ...
                        <div class="space-y-3 text-xs">
                            ...
                            <div class="pt-3 border-t border-slate-800 flex justify-between items-baseline">
                                ...
                                <span id="summary-grand-total" class="font-black text-xl text-blue-400">₱ 0.00</span>
                            </div>
                        </div>

                        <!-- Notice Box -->
                        <div class="bg-slate-800/80 rounded-xl p-3.5 border border-slate-700 text-[11px] text-slate-400 leading-relaxed">
                    ...
                    <!-- Direct Help Box -->
                    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm text-xs space-y-2">
...
<div id="catalog-modal" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col border border-slate-200 overflow-hidden">
        <div class="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
            <div>
                <h3 class="text-base font-bold text-slate-900">Select Item From Catalog</h3>
                <p class="text-xs text-slate-500">Pick any product from the Huenics master catalog.</p>
            </div>
            <button type="button" onclick="closeAddCatalogModal()" class="text-slate-400 hover:text-slate-600 p-1">
        ...
        <div class="p-4 border-b border-slate-200">
            <input type="text" id="catalog-search-input" onkeyup="filterCatalogModal()" placeholder="Search by name, SKU, or category..." 
                   class="w-full px-3.5 py-2 text-xs border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
        </div>

        <div class="overflow-y-auto p-4 space-y-2 divide-y divide-slate-100 flex-grow" id="catalog-list">
            ...
            <div class="catalog-item pt-2 pb-2 flex items-center justify-between gap-4" 
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <span class="font-bold text-xs text-slate-900">{{ $catProd->canonical_name }}</span>
                        <span class="bg-slate-100 text-slate-600 text-[10px] px-2 py-0.5 rounded font-mono">{{ $catProd->sku ?: 'SKU-00' }}</span>
                    </div>
                ...
                <div class="flex items-center gap-3 shrink-0">
                    <div class="text-right">
                        <div class="text-xs font-bold text-slate-900">₱ {{ number_format($catProd->selling_price ?: $catProd->default_price ?: 0, 2) }}</div>
                    </div>
                    <button type="button" 
                            onclick="insertCatalogItem({{ json_encode($catProd) }})"
                            class="bg-slate-900 hover:bg-blue-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition">
                        Select
                    </button>
                </div>
            </div>
        ...
        <div class="p-4 border-t border-slate-200 bg-slate-50 flex justify-end">
            <button type="button" onclick="closeAddCatalogModal()" class="bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold text-xs px-4 py-2 rounded-lg transition">
                Close
            </button>
        </div>
    </div>
...
<script>
            ...
            tr.innerHTML = `
                ...
                <td class="py-2.5 px-3">
                    ...
                    <input type="text" name="items[${index}][description]" value="${escapeHtml(item.description || '')}" required
                           ...
                           class="w-full px-2.5 py-1.5 border border-slate-200 rounded text-xs font-semibold focus:ring-1 focus:ring-blue-500 focus:outline-none bg-white">
                </td>
                <td class="py-2.5 px-3 text-center">
                    <input type="text" name="items[${index}][unit]" value="${escapeHtml(item.unit || 'pcs')}"
                           onchange="updateItemField(${index}, 'unit', this.value)"
                           class="w-16 text-center px-1.5 py-1.5 border border-slate-200 rounded text-xs focus:ring-1 focus:ring-blue-500 focus:outline-none bg-white">
                </td>
                <td class="py-2.5 px-3 text-center">
                    <input type="number" name="items[${index}][quantity]" value="${item.quantity}" min="0.01" step="any" required
                           ...
                           class="w-20 text-center px-2 py-1.5 border border-slate-300 rounded text-xs font-bold text-slate-900 focus:ring-1 focus:ring-blue-500 focus:outline-none bg-white">
                </td>
                <td class="py-2.5 px-3 text-right">
                    <input type="number" name="items[${index}][unit_price]" value="${item.unit_price}" min="0" step="0.01" required
                           ...
                           class="w-24 text-right px-2 py-1.5 border border-slate-300 rounded text-xs font-bold text-slate-900 focus:ring-1 focus:ring-blue-500 focus:outline-none bg-white">
                </td>
                <td class="py-2.5 px-3 text-right font-bold text-slate-900">
                    ₱ ${formatMoney(lineTotal)}
                </td>
                <td class="py-2.5 px-3 text-center">
                    <button type="button" onclick="deleteRow(${index})" class="text-slate-400 hover:text-red-600 transition p-1" title="Remove item">
    ...
    function insertCatalogItem(product) {
        currentItems.push({
            ...
            unit_price: parseFloat(product.selling_price || product.default_price || 0),
            line_total: parseFloat(product.selling_price || product.default_price || 0)
        });
